...
def test_request_haircut_submitted_with_auth_and_notification():
    ...
    request = request_haircut.execute(
        ...
        desired_date="2026-01-11T17:00:00Z",
        hair_length="long",
    ...
    assert notifier.messages == [
        {
            ...
            "body": "\n".join(
                [
                    ...
                    "Date souhaitée : 2026-01-11T17:00:00Z",
                    "Lieu : Saint-Cyprien",
                    "Longueur : long",
                    "Mèches : oui",
...
def test_request_haircut_can_skip_zone_coverage_validation():
    now = datetime(2026, 1, 10, 9, 0, tzinfo=timezone.utc)
    clock = FixedClock(now)
    ...
    assert request.location == "Unknown"


def test_request_haircut_can_skip_submission_notifications():
    now = datetime(2026, 1, 10, 9, 0, tzinfo=timezone.utc)
    clock = FixedClock(now)
...
def test_request_haircut_rejects_blocked_slot():
    ...
    blocked_slots = {
        provider_id: [
            (
                datetime(2026, 1, 11, 16, 0, tzinfo=timezone.utc),
                datetime(2026, 1, 11, 18, 0, tzinfo=timezone.utc),
            )
        ]
    }
